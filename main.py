#Instalacion del framework fastApi,codigo;

# -pip install fastapi-

#Instalacion del servidor Uvicorn,codigo;

# -pip install "uvicorn[standard]"-

#Importamos el framework fastapi a nuestro entorno de trabajo
from fastapi import FastAPI, APIRouter, HTTPException, status
#Importamos de la carpeta routers el codigo o las clases: "routers_5" y routers2_5 
from routers import Continentes_A, Regiones_Continentes_B, Regiones_C, Paises_Region_D, Atributos_Pais_E

#Creamos un objeto a partir de la clase FastAPI
app=FastAPI()

#Creamos un router a partir de la clase 
app.include_router(Continentes_A.router)
#Creamos un router a partir de la clase 
app.include_router(Regiones_Continentes_B.router)
#Creamos un router a partir de la clase 
app.include_router(Regiones_C.router)
#Creamos un router a partir de la clase
app.include_router( Paises_Region_D.router)
#Creamos un router a partir de la clase Finanzas
app.include_router(Atributos_Pais_E.router)
...
users_list=[User(code= "ABW",population=103000,life_expectancy=78.4,gnp=828,gnp_old=793,local_name="Aruba",government_form="Nonmetropolitan Territory of The Netherlands",head_of_state= "Willem-Alexander"),
                ...
                User(code= "PCN",population=50,life_expectancy=0,gnp=0,gnp_old=0,local_name="Pitcairn",government_form="Dependent Territory of the uK",head_of_state= "Elisabeth II"),
                User(code= "PER",population=25662000,life_expectancy=70,gnp=64140,gnp_old=65186,local_name="Peru/Piruw",government_form="Republic",head_of_state= "Valentin Paniagua Corazao"),
                User(code= "PHL",population=75967000,life_expectancy=67.5,gnp=65107,gnp_old=82239,local_name="Pilipinas",government_form="Republic",head_of_state= "Gloria Macapagal-Arroyo"),
                ...
                User(code= "POL",population=38653600,life_expectancy=73.2,gnp=151697,gnp_old=135636,local_name="Polska",government_form="Republic",head_of_state= "Aleksander Kwasniewski"),
...
#***Get con Filtro Path
@app.get("/continent/region/{code}", response_model=User, status_code=status.HTTP_200_OK)
async def routerPais(code:str):
    users=filter (lambda user: user.code == code, users_list)  #Función de orden superior
    try:
        return list(users)[0]
    except:
       raise HTTPException(status_code= status.HTTP_404_NOT_FOUND,detail="No se ha encontrado el país")    

 
#***Post
@app.post("/continent/region/{code}", response_model=User, status_code=201)
...
async def routerPais(user:User):
    ...
    #***Put
@app.put("/continent/region/{code}", response_model=User, status_code=status.HTTP_200_OK)
async def routerPais(user:User):
    ...
    if not found:
        ...
        #***Delete
@app.delete("/country/code/{code}", response_model=User, status_code=status.HTTP_200_OK)
async def routerPais(code:int):
...
#Utilizamos la (instancia) funcion get del framework FastAPI
@app.get("/")
...
async def imprimir():
      return{"Git_curso": "https://github.com7freddy-7777/Modelos-de-Desarrollo-Web.git"}
...
#Documentacion conRedocly:http://127.0.0.1:8000/redoc